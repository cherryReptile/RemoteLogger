...
func (p *ProvidersData) Create(db *sqlx.DB) error {
	...
	_, err := db.NamedExec(`INSERT INTO users_auth_providers_data (user_data, user_id, provider_id, created_at) 
								VALUES (:user_data, :user_id, :provider_id, :created_at)`, p)
...
func (p *ProvidersData) FindByUserUUIDAndProviderID(db *sqlx.DB, userUUID string, providerID uint) error {
	if err := db.Get(p, "SELECT * FROM users_auth_providers_data WHERE user_id=$1 AND provider_id=$2", userUUID, providerID); err != nil {
		return err
	}
	return nil
}
